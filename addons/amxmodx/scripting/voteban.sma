...
public SayIt(id)
{
  ...
  {
    client_print(id,print_chat,"[Public] votaban disabled")
    return 0
  }
  ...
  if( (Delay > Elapsed) && !is_user_admin(id) )
  {
    ...
    client_print(id,print_chat,"[AMXX] Вы должны ждать %d секунд до нового voteban может быть запущен", seconds)
    return 0
  }
  ...
  for(i=0; i<gi_TotalPlayers; i++)
  {
    ...
    if( is_user_admin(TempID))
    {
      if(!is_user_admin(id))
      {
        client_print(id,print_chat,"Существует администратора на сервере, голосование запрещен!")
        return 0
      }
    }
...
public outcom()
{
  ...
  {
    client_print(0,print_chat,"[Public] Недостаточное число избирателей запретить %s!", ga_PlayerName[gi_Sellection])
    return 0
  }
  else
  {
    ...
    {
      client_print(0,print_chat,"[AMXX] Голосование назад выстрелил в %s,запрещен к %d минут", ga_PlayerName[gi_VoteStarter], get_pcvar_num(gi_BanTime))
      ActualBan(gi_VoteStarter)
      log_amx("[AMXX] Голосование назад выстрелил в %s, eму запрещен для %d минут", ga_PlayerName[gi_VoteStarter], get_pcvar_num(gi_BanTime))
    }

    if( result >= get_pcvar_float(gf_Ratio) )
    {
      client_print(0,print_chat,"[Public] Проголосовать удалось, %s запрещен к %d минут", ga_PlayerName[gi_Sellection], get_pcvar_num(gi_BanTime))
      log_amx("[[Public] Проголосовать удалось, %s запрещен к %d минут", ga_PlayerAuthID[gi_Sellection], get_pcvar_num(gi_BanTime))
      ActualBan(gi_Sellection)
    }
    else
    {
      client_print(0,print_chat,"[Public] Проголосовать не удалось!")
      log_amx("[Public] Проголосовать не удалось.")
    }
  }
