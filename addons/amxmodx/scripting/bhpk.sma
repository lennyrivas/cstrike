...
public showInfo(taskid)
{
	...
	if (access(plrid, ADMIN_IMMUNITY) || access(plrid, ADMIN_RESERVATION))
		client_print(plrid, print_chat, "[BHPK] Проверка пинга отключена благодаря иммунитету...");
	else
	{
		client_print(plrid, print_chat, "[BHPK] Игроки с пингом выше чем %dед. будут кикнутыы!", get_hpk_ping_max());
...
kickPlayer(plrid)
{
	...
	client_cmd(plrid, "echo \"[BHPK] Извините но у вас слишком большой пинг, попробуйте зайти позже...\"; disconnect");
	server_cmd("addip 1 %s", ip);
	client_print(0, print_chat, "[BHPK] %s был отсоединен из за большого пинга!", name);
	return PLUGIN_CONTINUE;
} 
